	...
	<view>
		<view class="box-shawn">
			...
			<view style="border-bottom: 1upx solid #F4F4F4;display: flex;flex-direction: column;justify-content: center;">
				...
				<view>
					<myCheckboxGroup @selectChange="tabSwitchChange('superiority',$event)" :items="superiorityArr" v-if="superiorityArr.length>0"></myCheckboxGroup>
				</view>
			</view>
			...
			tabSwitchChange:function(label,index){
			 ...
			 pickerChange:function(label,event){
			    ...
			    async submit(){
					...
					const res = await this.$http.post('/cm/rivalAdd',{
						data: this.form
					})
					uni.hideLoading()
					console.log(res)
