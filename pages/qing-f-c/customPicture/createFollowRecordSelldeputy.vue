	...
	<view>
		<view class="box-shawn">
			...
			<view class="content-1">
				...
				<view class='items'>
					<myPickerPart :items="selectList.stabilityList" @mychange="pickerChange('stabilityCode',$event)" :firstLabel='-1'></myPickerPart>
				</view>
			</view>
			<view class="content-1">
	...
	export default {
		...
		methods:{
			...
			async submit(){
				...
				const res = await this.$http.post('/portrayal/seller_add',{data:data})
				uni.hideLoading()
				...
				uni.redirectTo({
					url: `./followRecordDetailSeller?recordCode=${res.data.data.msg}`
				})
