	...
	<view>
		...
		<view>
		  <block v-for="(item, index) in customerList" :key="index">
		    <view class="list flex_c box_shadow" @click.stop="toClientDetail(item.companyCode)" :data-id="item.companyCode" :data-index="index">
		      ...
		      <view :class="(compileing?'wid_610':'wid_670')">
		        ...
		        <view class="flex_c mt_20">
		          ...
		          <view class="fs_14 ">
		            <text class="mr_30">{{item.regionName||''}}</text>
					<text>{{item.companyTypeName||''}}</text>
		            <text style="padding-left: 30upx">{{item.buyOrSellCount||''}}个联系人</text>
		            
		          </view>
		        </view>
		        <view class="flex_c mt_20">
					  <image src="/static/images/qingfc/application/contacts.png" class="title_img" mode="aspectFit"></image>
					  <view class="fs_14">{{item.realName||''}}<text style="padding-left: 30upx">{{item.phone||''}}</text></view>
		        </view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			toClientDetail:function(companyCode){
				uni.navigateTo({
					url: './selectProduct?companyCode='+companyCode,
					success: res => {},
