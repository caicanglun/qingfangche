<template>
	...
	<view>
			...
			<view class="record-wrap " @tap="toDetail(item.recordCode)">
				<view class="flex_sb">
					<view class="flex_column" style="line-height: 25px;">
						...
						<view class="followContent" v-if="item.realName"><text style="color: #000000">{{item.realName||''}}</text></view>
					    <!-- <view class="followContent" v-if="item.buyDeputyRealName"><text style="color: #000000">{{item.buyDeputyRealName||''}}</text></view> -->
				...
				<view style="display: flex;justify-content: space-between;font-size:13px;margin-top: 15upx;">
					...
					<view>负责人：{{item.principalRealName || '待分配'}}</view>
				</view>
			</view>
		...
		<view class="fixed_right_bottom box_shadow"  @tap="addFollow" v-if="isShowButton">
		  <view>新建</view>
		  <view>订单</view>
		</view>
