...
<view>
	<view class="form_box box_shadow">
		...
		<view class="flex_c list" @tap="selectSeller">
		  <view class="list_right">公司：</view>
		  <view :class="(companyName==''?'color_909090 wid_400':'wid_400')">{{companyName==''?"请选择":companyName}}</view>
...
<!-- ------------------------------------	 -->
  ...
  <!-- - - - - - - - - -- - -- - - - - - - - - - --  -- -->
	...
	<!-- ------------------------------------------------ -->
		...
		<view class="form_box box_shadow">
			<view class="flex_c" style="padding: 0 30upx;">
				<view style="width: 175upx;">品质定位</view>
				<view style="width: 100%;">
					<myPickerSmall @mychange="tabSwitchChange('qualityPosition',$event)" :firstLabel='qualityPositionIndex' :items="qualityPosition"></myPickerSmall>
				</view>
			</view>
...
<script>
	...
	import myPickerSmall from "@/components/myPickerPartProps.vue";
	  ...
	  methods:{
		 ...
		 handleSeason:function(e){
		  ...
		  async submit(){
			  ...
			  const res = await this.$http.post('/shop/goodsAdd',{
				  data: _data
			  })
			  console.log(res)
			  uni.hideLoading()
			  ...
			  let lastTwo =  pages[pages.length - 3]; //上二个页面，，mainShow
			  let lastThree = pages[pages.length - 4]; //上三个页面，，customer-details
			  lastTwo.$vm.companyCode = this.companyCode
			  lastThree.$vm.companyCode = this.companyCode
