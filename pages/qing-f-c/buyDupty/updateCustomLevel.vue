	...
	export default {
		...
		methods:{
			...
			async submit(){
				...
				let data={
					companyCodeList:[this.companyCode],    //公司编码数组
					companyLevel: this.statusCode	     //等级
				}
				...
				const res = await this.$http.post('/cm/update_level',{data:data})
				uni.hideLoading()
				uni.navigateBack({
					delta: -1
				});
