<template>
	<view>
		...
		<view class="box box_shadow">
		  ...
		  <!-- ----------------------------- -->
		  <myPicker @mychange="tapPickerEvent('operateCapital',$event)" :items="operateCapital" name="资金状况" :firstLabel="capitalName"></myPicker>
		  <view class="title">好：资金充足,随时付款；良：按期付款；一般：偶尔拖；差：经常拖</view>
		  <myPicker @mychange="tapPickerEvent('operateWom',$event)" :items="operateWom" name="市场口碑" :firstLabel="womName"></myPicker>
		  <myPicker @mychange="tapPickerEvent('operateCredit',$event)" :items="operateCredit" name="公信度级别" :firstLabel="creditName"></myPicker>
		  <myPicker @mychange="tapPickerEvent('operateOperation',$event)" :items="operateOperation" name="运营状况" :firstLabel="operationName"></myPicker>
		</view> 
...
<script>
	...
	export default {
		...
		onShow:function(){
			...
			//旺季时间，字符串拆成数组
			let season = _operateCode.highSeasonTime.split(',')
			season.forEach((s) =>{
		...
		methods:{
			...
			submit:function(e){
				...
				JsyServer.operationUpdate(_data).then(res => {
				  console.log(res);
