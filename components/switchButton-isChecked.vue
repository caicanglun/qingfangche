	...
	<view>
		 <view class="flex_c">
			 <block v-for="item in items" :key="item.id">
			   <view :class="'unit ' + (returnIndex(item.id)==0?'borderleft':'') + ' ' + (returnIndex(item.id)==1?'borderright':'') + ' ' + (index==item.isChecked?'unitselect':'unitunselect')"
				@tap="buttonChange(item.id)" :data-index="item.id"><text style="padding:0upx 10upx;">{{item.label}}</text></view>
			 </block>
		 </view>
	</view>
</template>
