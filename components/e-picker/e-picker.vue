...
<template>
	<view>
		<picker mode="multiSelector" :range="dateTimeArr" @change="pickerChange" :value="value" @columnchange="columnchange">
	...
	export default {
		...
		methods: {
			...
			getDateTimeValue(arr) {
				...
				return `${y}-${this.format(M)}-${this.format(d)} ${this.format(h)}:${this.format(m)}:${this.format(s)}`
			},
			format(i) {
				return i >= 10 ? i : '0' + i
			},
